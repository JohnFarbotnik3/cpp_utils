...
namespace utils::vector_util {
	...
	template<class T> void vec_copy(vector<T>& dst, const vector<T>& src, const int dst_beg, const int src_beg, const int len) {
		// TODO - assertions.
		memcpy(dst.data() + dst_beg, src.data() + src_beg, len * sizeof(T));
	}
	...
	template<class T> void vec_fma_fn(vector<T>& vec, const T mult, const T add, const int beg, const int end) {
		// TODO - assertions.
		for(int x=beg;x<end;x++) vec[x] = vec[x] * mult + add;
	}
	template<class T> void vec_fma_mt(vector<T>& vec, const T mult, const T add, const int beg, const int end, const int n_threads) {
		vector<std::thread> threads;
		const int len = end - beg;
		for(int x=0;x<n_threads;x++) {
			const int x0 = ((x+0) * len) / n_threads + beg;
			const int x1 = ((x+1) * len) / n_threads + beg;
			threads.push_back(std::thread(vec_fma_fn<T>, std::ref(vec), mult, add, x0, x1));
		}
		for(int x=0;x<n_threads;x++) threads[x].join();
	}
